...
if($total_quartos > 0){
	...
	$numero_quarto = @$res2[0]['numero'];
		...
		//ACRESCENTAR ESSA CONSUTA PARA PODER VERIFICAR RESERVAS QUE ESTIVEREM ANTES DO CHECKIN E DEPOIS DO CHECKOUT DE UMA EXISTENTE, OU SEJA, QUE PEGA TODO O PERIODO DELA
			...
			$query2 = $pdo->query("SELECT * from reservas where (quarto = '$id_quarto' and check_in < '$dataFinal' and check_out >= '$dataFinal') or (quarto = '$id_quarto' and check_in > '$dataInicial' and check_out <= '$dataFinal') order by id desc");
